...
zone = Zone(
    ...
    missions=[
        ...
        Mission(
            ...
            description="Define una clase `Nodo` con valor, izquierda y derecha. Crea este árbol:\n      2\n     / \\\n    1   3\nY haz un recorrido in-order que muestre los valores.",
            execution_mode="script",
            code_template="class Nodo:\n    def __init__(self, valor):\n        self.valor = valor\n        self.izquierda = None\n        self.derecha = None\n\ndef in_order(nodo):\n    if nodo:\n        in_order(nodo.izquierda)\n        print(nodo.valor)\n        in_order(nodo.derecha)\n\nraiz = Nodo(2)\nraiz.izquierda = Nodo(1)\nraiz.derecha = Nodo(3)\n\nin_order(raiz)",
        ...
        Mission(
            ...
            description="Implementa un grafo simple usando dict de listas de adyacencia.\nDefine: agregar_amigo(persona1, persona2) y mostrar_red(persona) que muestre sus conexiones.\nCrea esta red:\n  Ana — Bob — Carlos — Diana\n  Ana — Diana\nY muestra la red de Ana.",
            execution_mode="script",
            code_template="class GrafoSocial:\n    def __init__(self):\n        self.adyacencia = {}\n\n    def agregar_amigo(self, p1, p2):\n        if p1 not in self.adyacencia:\n            self.adyacencia[p1] = []\n        if p2 not in self.adyacencia:\n            self.adyacencia[p2] = []\n        self.adyacencia[p1].append(p2)\n        self.adyacencia[p2].append(p1)\n\n    def mostrar_red(self, persona):\n        amigos = self.adyacencia.get(persona, [])\n        for a in sorted(amigos):\n            print(a)\n\ng = GrafoSocial()\ng.agregar_amigo('Ana', 'Bob')\ng.agregar_amigo('Bob', 'Carlos')\ng.agregar_amigo('Carlos', 'Diana')\ng.agregar_amigo('Ana', 'Diana')\ng.mostrar_red('Ana')",
            test_cases=[TestCase(input="", expected="Bob\nDiana")],
